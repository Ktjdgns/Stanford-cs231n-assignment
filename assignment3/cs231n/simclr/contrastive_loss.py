...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i @ z_j.T) / (norm_i * norm_j)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if k == i:
                continue
            d1 += torch.exp(sim(z_k, out[i])/tau)

        for i in range(2*N):
            if k + N == i:
                continue
            d2 += torch.exp(sim(z_k_N, out[i]) / tau)

      
        loss1 = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / d1)
        loss2 = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / d2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    mm = torch.sum(out_left * out_right, dim=1, keepdim=True)

    pos_pairs = mm / (norm_left * norm_right)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = torch.cat([pos_sim, pos_sim], dim=0)
